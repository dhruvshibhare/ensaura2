...
const limited = listProductsByCategory('limited');
            ...
            <div className={`grid gap-4 sm:gap-8 ${viewMode === 'grid' ? 'grid-cols-2 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
              {limited.map((item, index) => (
                <motion.div
                  ...
                  whileHover={{ y: -5 }}
                >
                  <Link href={`/products/${item.id}`} className={`block relative ${viewMode === 'list' ? 'w-1/3' : ''}`}>
                  ...
                  <div className={`p-6 ${viewMode === 'list' ? 'w-2/3' : ''}`}>
                    ...
                    {viewMode === 'list' && (
                      <p className="text-gray-600 mb-4 line-clamp-2">{item.description.split('\n')[0]}</p>
                    )}
